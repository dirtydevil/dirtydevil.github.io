...
prx.types.ios7_statusbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '#'+_id+' .ios7-statusbar-signal-dot { width: '+Math.floor(signalDotDims*(percent))+'px; height: '+Math.floor(signalDotDims*(percent))+'px; margin-top: '+Math.floor(signalDotMargin*percent)+'px; margin-bottom: '+Math.floor(signalDotMargin*percent)+'px; background-color: '+getColor(item.foregroundColor)+'; border-color: '+getColor(item.foregroundColor)+'; }';
		cR += '#'+_id+' .ios7-statusbar-battery-life-icon { width: '+Math.floor(batteryWidth*percent)+'px; height: '+Math.floor(batteryHeight*percent)+'px; margin-top: '+Math.floor(batteryMarginTop*percent)+'px; background-color: '+getColor(item.foregroundColor)+'; border-color: '+getColor(item.foregroundColor)+'; }';
		cR += '#'+_id+' .ios7-statusbar-battery-life-icon-battery-pole-inner { width: '+Math.floor(batteryInnerDims*(percent))+'px; height: '+Math.floor(batteryInnerDims*(percent))+'px; left: '+Math.floor(batteryInnerLeftPos*(percent))+'px; top: '+Math.ceil(batteryInnerTopPos*(percent))+'px; background-color: '+getColor(item.foregroundColor)+'; }';
...
prx.types.ios8_searchbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#'+_id+'  input { color: '+getColor(item.textColor)+'; font-size: '+item.textSize+'px; '+getFontCssFromFontFamily(item.textFont)+' '+ _props + ' ;}';
		cR += '#'+_id+' .ios8-searchbar-faux-input { background-color: '+getColor(item.backgroundColor)+'; color: '+getColor(item.textColor)+'; font-size: '+item.textSize+'px; '+getFontCssFromFontFamily(item.textFont)+' line-height: '+_dims.height+'px; '+ _props + ' }';
		cR += '#'+_id+' .ios8-searchbar-faux-input-overlay .placeholder-input, #'+_id+' .ios8-searchbar-input-overlay .placeholder-input { color: '+getColor(item.placeholderColor)+';  font-size: '+item.textSize+'px;}';
		if(item.imgSrc.url != '') {
			cR += '#' + _id + ' .ios8-searchbar-icon { width: '+item.textSize+'px; background-image: url('+getAssetUrl(item.imgSrc)+');}';
			cR += '#' + _id + ' .ios8-searchbar-icon-faux { width: '+item.textSize+'px;}';
		}
		cR += '#'+_id+' .ios8-searchbar-input-flex { background-color: '+getColor(item.backgroundColor)+'; border: '+item.borderWidth+'px solid '+getColor(item.borderColor)+'; border-radius: '+item.borderRadius+'px; }';
		cR += '#' + _id + ' .ios8-searchbar-icon-erase { width: '+(_dims.height*0.4)+'px; background-image: url('+getAssetUrl(item.imgSrc2)+'); color: '+getColor(item.placeholderColor)+';}';
		cR += '</style>'
...
prx.types.ios8_map_tag = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .ios8-map-tag-wrapper { background-color: '+getColor(item.backgroundColor)+'; border-radius: '+borderRadius+'px; }';
		cR += '#' + _id + ' .ios8-map-tag-point { border-top-color: '+getColor(item.backgroundColor)+';}';
		...
		cR += '#' + _id + ' .ios8-map-tag-text { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; '+_props+'}';
		cR += '#' + _id + ' .ios8-map-tag-duration { color: '+getColor(item.durationColor)+'; '+getFontCssFromFontFamily(item.durationFont)+' font-size: '+item.durationSize+'px; '+_duration_props+'}';
		
		cR += '#' + _id + ' .ios8-map-tag-left-info { width: '+dims.height+'px; background-color: '+getColor(item.durationBgColor)+'; border-radius: '+borderRadius+'px 0 0 '+borderRadius+'px;}';
		if(item.imgSrc.url != '') {
			cR += '#' + _id + ' .ios8-map-tag-icon-left {  background-image: url('+getAssetUrl(item.imgSrc)+'); }';
		}
		if(item.imgSrc2.url != '') {
			cR += '#' + _id + ' .ios8-map-tag-icon-right { background-image: url('+getAssetUrl(item.imgSrc2)+'); }';
		}
		cR += '</style>';
...
prx.types.ios8_message_bubble = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>'
		cR += '#' + _id + ' .ios8-message-bubble-wrapper { background-color: '+getColor(item.backgroundColor)+'; text-align: '+item.textAlign+'; }';
		cR += '#' + _id + ' .ios8-message-bubble-text { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; '+_props+'}';
		if(item.thumbnail.url != '') {
			cR += '#' + _id + ' .ios8-message-bubble-thumb { background-image: url('+getAssetUrl(item.thumbnail)+');}';
		}
		cR += '#' + _id + ' .ios8-message-bubble-tip { border-color: '+getColor(item.backgroundColor)+';}';
		cR += '</style>'
